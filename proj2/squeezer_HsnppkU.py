...
def squeezer_explicit(t, y):
    ...
    gqq = zeros(6)
    gqq[0] = - rr * cobe * v[0] ** 2 + d * cobeth * (v[0] + v[1]) ** 2 +\
             ss * siga * v[2] ** 2
    gqq[1] = - rr * sibe * v[0] ** 2 + d * sibeth * (v[0] + v[1]) ** 2 -\
             ss * coga * v[2] ** 2
    gqq[2] = - rr * cobe * v[0] ** 2 + d * cobeth * (v[0] + v[1]) ** 2 +\
             e * siphde * (v[3] + v[4]) ** 2 + zt * code * v[4] ** 2
    gqq[3] = - rr * sibe * v[0] ** 2 + d * sibeth * (v[0] + v[1]) ** 2 -\
             e * cophde * (v[3] + v[4]) ** 2 + zt * side * v[4] ** 2
    gqq[4] = - rr * cobe * v[0] ** 2 + d * cobeth * (v[0] + v[1]) ** 2 +\
             zf * coomep * (v[5] + v[6]) ** 2 + u * siep * v[6] ** 2
    gqq[5] = - rr * sibe * v[0] ** 2 + d * sibeth * (v[0] + v[1]) ** 2 +\
             zf * siomep * (v[5] + v[6]) ** 2 - u * coep * v[6] ** 2


    A = hstack((vstack((m, gp)), vstack((gp.T, zeros((6, 6))))))
    b = hstack((ff[0:7], - gqq))
    ...
    w_lamb = linalg.solve(A, b)

    w = w_lamb[:7]

    global lamb_global

    lamb_global = w_lamb[7:]
